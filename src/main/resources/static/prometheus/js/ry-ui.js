...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
            	var defaults = {
                    ...
                    showToggle: true,
                    showExport: false,
                    clickToSelect: false,//默认为false不启用点击选中行，设为true启用
                    mobileResponsive: true,
                    rememberSelected: false,//默认为false不启用翻页记住前面的选择，设为true启用
        		    fixedColumns: false,//默认为false禁用冻结列，设为true启用冻结列（左侧）
        		    fixedNumber: 0,
        		    rightFixedColumns: false,//默认为false禁用冻结列，设为true启用冻结列（右侧）
        		    rightFixedNumber: 0,
                ...
                /*//表格高度自适应
				if($(".container-div").height() != null && $(".search-collapse").height() != null && $(".btn-group-sm").height() != null){
					$(".fixed-table-container").height($(".container-div").height()- $(".search-collapse").height()- $(".btn-group-sm").height() - 150);
				}*/
            },
			...
			// 根据id回显数据名称
			selectDataName: function(datas, value) {
				var actions = [];
				$.each(datas, function(index, data) {
					if (data.dataId == ('' + value)) {
						actions.push("<span class='badge'>" + data.dataName + "</span>");
						return false;
					}
				});
				return actions.join('');
        ...
        operate: {
			...
			detailWithUrl: function(url, title, width, height) {
				var _url = $.common.isEmpty(url) ? $.table._option.detailUrl : url;
				var _width = $.common.isEmpty(width) ? "800" : width;
				var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
				...
				layer.open({
					...
					maxmin: true,
					shade: 0.3,
					title: $.common.isEmpty(title) ? "详细" : title,
					content: _url,
					btn: ['关闭'],
					// 弹层外区域关闭
					shadeClose: true,
			...
			//指令下发成功后 调用进度条
			commInstr: function(url, data, callback) {
				var config = {
					url: url,
					type: "post",
					dataType: "json",
...
// 回调弹出 显示 进度条页面
function callbackShowPercent(result) {
//  还是要判断本身的指令有没有存数据库,报错要给反馈
	if (result.code == web_status.SUCCESS) {
		// callbackShowPercent(result);
	} else if (result.code == web_status.WARNING) {
		$.modal.alertWarning(result.msg)
	...
	var ids = result.commListIds;
	if(ids != "" && ids != undefined){
		var url = ctx + 'common/showPercent/' + ids.join(',');
		layer.open({
			type: 2,
			area: ['800px', '120px'],
			fix: false,
			...
			cancel: function(index, layero){
				...
				if (parent.$.table._option.type == table_type.bootstrapTable) {
					...
					$.modal.close();
					parent.$.treeTable.refresh();
				} else if ($.table._option.type == table_type.bootstrapTable) {
					$.table.search();
